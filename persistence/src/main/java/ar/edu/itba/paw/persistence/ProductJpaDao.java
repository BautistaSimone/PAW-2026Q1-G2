...
public class ProductJpaDao implements ProductDao {
    ...
    public PaginatedResult<Product> findProducts(final ProductSearchCriteria criteria) {
        ...
        if (!criteria.getCategoryIds().isEmpty()) {
            whereSql.append(
                    " AND EXISTS (SELECT 1 FROM products_categories pc WHERE pc.product_id = p.product_id AND pc.category_id IN :categoryIds)");
        ...
        if (!criteria.getConditionBuckets().isEmpty()) {
            ...
            for (ConditionBucket bucket : criteria.getConditionBuckets()) {
                ...
                switch (bucket) {
                    ...
                    case MUY_BUENO -> whereSql.append(
                            "( (p.sleeve_condition + p.record_condition)/2 >= 8.0 AND (p.sleeve_condition + p.record_condition)/2 < 9.0 )");
                    case BUENO -> whereSql.append(
                            "( (p.sleeve_condition + p.record_condition)/2 >= 7.0 AND (p.sleeve_condition + p.record_condition)/2 < 8.0 )");
        ...
        if (ids.isEmpty()) {
            return new PaginatedResult<>(Collections.emptyList(), criteria.getPage(), criteria.getPageSize(),
                    totalCount);
        }
